...
function hexagonPoints(x, y, radius) {
  return Array.from({ length: 6 }, (_, corner) => {
    ...
    return `${(x + radius * Math.cos(angle)).toFixed(3)},${(y + radius * Math.sin(angle)).toFixed(3)}`;
  }).join(' ');
}
function renderHeatmap(report) {
  ...
  const thresholds = values.length ? [
    values[Math.floor((values.length - 1) / 4)],
    values[Math.floor((values.length - 1) / 2)],
    values[Math.floor((values.length - 1) * 3 / 4)]
  ] : [0, 0, 0];
  ...
  for (let row = 0; row < rows; ++row) {
    ...
    for (let column = 0; column < columns; ++column) {
      const x = hexWidth * (column + offset), y = radius * (1 + 1.5 * row);
      const dwell = populated.get(`${column},${row}`) || 0;
      const points = hexagonPoints(x, y, radius);
      if (!dwell) polygons.push(`<polygon class="hexbin-empty" points="${points}"/>`);
      else {
        const band = dwell <= thresholds[0] ? 0 : dwell <= thresholds[1] ? 1 : dwell <= thresholds[2] ? 2 : 3;
        polygons.push(`<polygon class="hexbin" points="${points}" fill="${colors[band]}" stroke="${colors[band]}"/>`);
      }
    }
  }
  return `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Mouse dwell-time heatmap" preserveAspectRatio="xMidYMid meet">${polygons.join('')}</svg>`;
}
...
function render(report) {
  ...
  const averageApm = actions / minutes;
