...
function render(report) {
  ...
  const bins = report.hexbins || [], values = bins.map(bin => Number(bin.dwell_ms || 0)).filter(Boolean).sort((a, b) => a - b), binValues = new Map(bins.map(bin => [`${bin.column}:${bin.row}`, Number(bin.dwell_ms || 0)]));
  const palette = ['#3b82f6', '#06b6d4', '#facc15', '#ef4444'];
  const columns = Math.max(1, Math.ceil(100 / (Math.sqrt(3) * grid.radius)) + 1), rows = Math.max(1, Math.ceil((100 / grid.aspect) / (1.5 * grid.radius)) + 1);
  for (let row = 0; row < rows; ++row) for (let column = 0; column < columns; ++column) {
    const offset = row & 1 ? .5 : 0, x = Math.sqrt(3) * grid.radius * (column + offset), y = grid.radius * (1 + 1.5 * row), value = binValues.get(`${column}:${row}`) || 0;
    const rank = values.findLastIndex(entry => entry <= value), band = Math.min(3, Math.floor(Math.max(0, rank) * 4 / Math.max(1, values.length)));
    const hex = document.createElement('i'); hex.className = 'hexbin';
    hex.style.left = `${x}%`; hex.style.top = `${y * grid.aspect}%`; hex.style.width = `${2 * grid.radius}%`; hex.style.height = `${2 * grid.radius * grid.aspect}%`; hex.style.background = value ? palette[band] : '#000'; hex.style.opacity = value ? '.88' : '.15'; heatmap.append(hex);
  }
